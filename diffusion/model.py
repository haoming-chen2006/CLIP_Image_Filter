...
# FlashAttention compatibility layer
try:
    from torch.nn.attention import sdpa_kernel  # PyTorch ≥ 2.3
except ImportError:
    from torch.backends.cuda import sdp_kernel as sdpa_kernel  # Fallback for ≤ 2.2
    ...
    def forward(self, x):
        ...
        if self.flash:
            try:
                ...
                print("[Fallback] Old PyTorch: falling back to default SDPA context.")
                out = self.attend(q, k, v)
        else:
            out = self.attend(q, k, v)
...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        try:
            ...
            print(f"[Dataset] Warning: Failed to load image {path}: {e}")
            # Option 1: try next image
            return self.__getitem__((index + 1) % len(self))
            # Option 2 (alternative): return a dummy image instead
